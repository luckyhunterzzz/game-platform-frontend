...
import AdminEventsPageClient from '@/components/events/AdminEventsPageClient';
    ...
    <div className="flex min-h-screen flex-col bg-[var(--background)] font-sans text-[var(--foreground)]">
      ...
      {!authenticated ? (
        <main className="flex flex-1 flex-col items-center px-4 py-10">
          <div className="mx-auto flex w-full max-w-3xl flex-1 items-center justify-center">
            <div className="w-full rounded-[2rem] border border-[var(--border)] bg-[var(--surface)] p-8 text-center shadow-[0_24px_70px_rgba(0,0,0,0.18)]">
              <h1 className="text-3xl font-black text-[var(--foreground)]">{messages.profile.signInTitle}</h1>
              <p className="mt-4 text-sm leading-7 text-[var(--foreground-soft)]">{messages.profile.signInDescription}</p>
              <div className="mt-6 flex flex-wrap justify-center gap-3">
                <button type="button" onClick={login} className="rounded-xl bg-blue-600 px-4 py-2 text-sm font-medium text-white transition hover:bg-blue-700">{loginLabel}</button>
                <button type="button" onClick={navigateHome} className="rounded-xl border border-[var(--border)] bg-[var(--surface-strong)] px-4 py-2 text-sm text-[var(--foreground)] transition hover:bg-[var(--surface-hover)]">{messages.home.menuPageOne}</button>
              </div>
            </div>
          </div>
        </main>
        ...
        <main className="flex flex-1 flex-col items-center px-4 py-10">
          <div className="mx-auto flex w-full max-w-3xl flex-1 items-center justify-center">
            <div className="w-full rounded-[2rem] border border-[var(--border)] bg-[var(--surface)] p-8 text-center shadow-[0_24px_70px_rgba(0,0,0,0.18)]">
              <h1 className="text-3xl font-black text-[var(--foreground)]">{messages.heroes.accessDeniedTitle}</h1>
              <p className="mt-4 text-sm leading-7 text-[var(--foreground-soft)]">{messages.heroes.accessDeniedDescription}</p>
              <div className="mt-6 flex flex-wrap justify-center gap-3">
                <button type="button" onClick={navigateHome} className="rounded-xl border border-[var(--border)] bg-[var(--surface-strong)] px-4 py-2 text-sm text-[var(--foreground)] transition hover:bg-[var(--surface-hover)]">{messages.home.menuPageOne}</button>
              </div>
            </div>
