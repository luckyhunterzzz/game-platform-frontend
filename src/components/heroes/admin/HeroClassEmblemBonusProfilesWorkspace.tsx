...
export default function HeroClassEmblemBonusProfilesWorkspace() {
  ...
  const [catalogPage, setCatalogPage] =
    useState<CatalogResponseDto<HeroClassEmblemBonusProfileResponseDto> | null>(null);
  ...
  const handleLoadMore = async () => {
    ...
    try {
      ...
      setListError(error instanceof Error ? error.message : 'Failed to load more profiles');
    } finally {
          ...
          {loadingList ? (
            ...
            <div className="space-y-3">
              ...
              {catalogPage?.hasNext ? (
                ...
                >
                  {loadingMore ? t.loadingList : heroLocale === 'RU' ? 'Показать еще' : 'Load more'}
                </button>
              ) : null}
            </div>
          )}
