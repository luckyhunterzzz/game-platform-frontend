...
export default function EventImageUploadField({
  ...
  onChange: (value: Required<EventImageValue>) => void;
  ...
  useEffect(() => {
    const nextFileName = value.imageObjectKey ? value.imageObjectKey.split('/').at(-1) ?? value.imageObjectKey : null;
  ...
  return (
    <div className="space-y-3">
      <label className="block text-sm font-medium text-[var(--foreground-soft)]">{text.label}</label>
      ...
      {value.imageUrl ? (
        <div className="overflow-hidden rounded-2xl border border-[var(--border)] bg-[var(--surface)]">
          ...
          <img src={value.imageUrl} alt={fileName ?? 'Event image preview'} className="max-h-56 w-full object-contain bg-black/10" />
